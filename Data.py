...
class Point:
	...
	def __str__(self):
		string = "%.3f %.3f %.3f" % (self.lon, self.lat, self.value)
...
class Points:
	...
	def __init__(self, points=[], filename=None):
		...
		if filename != None: load(filename) #if given a file, loads it
	...
	def residMatrix(self, shape, mmm=None):
		return np.reshape(self.residArray(mmm), shape)
	...
	def covMatrix(self, model, alpha):
		c0  = self.var()
		mat = [model(p1.distance(p2), alpha, c0) for p1 in self.points for p2 in self.points]
	...
	def covVector(self, point, model, alpha):
		c0  = self.var()
		return np.asarray([model(point.distance(p1), alpha, c0) for p1 in self.points])
